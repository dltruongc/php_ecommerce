...
$datHang = new DatHang($conn);

$dsDatHang = $datHang->xemDatHang();

if (isset($_POST["add"])) {
    $err = array_merge($err, $datHang->themDatHang($_POST["MSKH"], $_POST["NgayDH"], $_POST["TrangThai"], $_POST["MSNV"]));

    if (count($err) > 0) {
        $err = implode(", ", $err);
        echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
    } else header("Location: " . ROOT_URL . "/dashboard_bill.php");
} else
    if (isset($_POST["delete"])) {
        $err = array_merge($err, $datHang->xoaDatHang($_POST["SoDonDH"]));

        if (count($err) > 0) {
            $err = implode(", ", $err);
            echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
        } else
            header("Location: " . ROOT_URL . "/dashboard_bill.php");
    } else
        if (isset($_POST["update"])) {
            $err = array_merge($err, $datHang->capNhatDatHang($_POST["SoDonDH"], $_POST["MSKH"], $_POST["NgayDH"], $_POST["TrangThai"], $_POST["MSNV"]));
            if (count($err) > 0) {
                $err = implode(", ", $err);
                echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
            } else {
