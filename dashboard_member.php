...
$dsNhanVien = $nhanVien->xemNhanVien();

if (isset($_POST["add"])) {
    $err =array_merge($err, $nhanVien->themNhanVien($_POST["HoTenNV"], $_POST["ChucVu"], $_POST["SoDienThoai"], $_POST["DiaChi"]));

    if (count($err) > 0) {
        $err = implode(", ", $err);
        echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
    } else header("Location: " . ROOT_URL . "/dashboard_member.php");
} else
    if (isset($_POST["delete"])) {
        $err = array_merge($err, $nhanVien->xoaNhanVien($_POST["MSNV"]));

        if (count($err) > 0) {
            $err = implode(", ", $err);
            echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
        } else
            header("Location: " . ROOT_URL . "/dashboard_member.php");
    } else
        if (isset($_POST["update"])) {
            $err = array_merge($err, $nhanVien->capNhatNhanVien($_POST["MSNV"], $_POST["HoTenNV"], $_POST["ChucVu"], $_POST["DiaChi"], $_POST["SoDienThoai"]));
            if (count($err) > 0) {
                $err = implode(", ", $err);
                echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
            } else {
                ...
                <div class="mh-100 overflow-auto">
                    <table class="table">
                        ...
                        <tr class="table-active">
                            <form action="dashboard_member.php" method="post">
                                <th scope="row">?</th>
                                <td>
                                    <input class="form-control form-text" type="text" name="HoTenNV"
                                           placeholder="Họ Tên Nhân viên">
                                </td>
                                <td>
                                    <input class="form-control form-text" type="text" name="ChucVu"
                                           placeholder="Chức vụ">
                                </td>
                                <td>
                            ...
                            echo "
                                    ...
                                    <td>
                                        ...
                                        <div role='group' aria-label='Basic example'> 
                                            <input type='hidden' name='MSNV' value='$nv[0]' >
