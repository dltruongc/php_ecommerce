...
$dsKhachHang = $khachHang->xemKhachHang();

if (isset($_POST["add"])) {
    array_merge($err, $khachHang->themKhachHang($_POST["HoTenKH"], $_POST["DiaChi"], $_POST["SoDienThoai"]));

    if (count($err) > 0) {
        $err = implode(", ", $err);
        echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
    } else
        header("Location: " . ROOT_URL . "/dashboard_customer.php");
} else
    if (isset($_POST["delete"])) {
        array_merge($err, $khachHang->xoaKhachHang($_POST["MSKH"]));
        ...
        if (isset($_POST["update"])) {
            array_merge($err, $khachHang->capNhatKhachHang($_POST["MSKH"], $_POST["HoTenKH"], $_POST["DiaChi"], $_POST["SoDienThoai"]));
            if (count($err) > 0) {
                $err = implode(", ", $err);
                echo "<script type='text/javascript'>alert('Có lỗi: $err')</script>";
            } else {
            ...
            <div class="col-sm-12 section-t2">
                <div class="mh-100 overflow-auto">
                    <table class="table table-hover">
                        ...
                        <form action="dashboard_customer.php" method="post">
                            <input type='hidden' name='add' value='$kh[0]'>
                        ...
                        foreach ($dsKhachHang as $kh) {
                            echo "
                                    <tr>
                                    <form class='mr-1' action='dashboard_customer.php' method='post'>
                                        <input type='hidden' name='MSKH' value='$kh[0]' >
                                        <th scope=\"row\">$kh[0]</th>
                                        <td><input class='form-control form-text' type='text' name='HoTenKH' value='$kh[1]'></td>
                                        <td><input class='form-control form-text' type='text' name='DiaChi' value='$kh[2]'></td>
                                        <td><input class='form-control form-text' type='text' name='SoDienThoai' value='$kh[3]'></td>
